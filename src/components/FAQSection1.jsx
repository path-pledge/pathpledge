...
export const FAQSection1 = () => {
  ...
  return (
    <section className="bg-white py-20 px-4 sm:px-6 lg:px-20 section-fade-in">
      <div className="max-w-6xl mx-auto text-center">
        <h2 className="text-4xl sm:text-5xl font-extrabold text-gray-900 mb-4 leading-tight font-serif">
          Frequently Asked <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D9070A] to-[#ff9900]">Questions</span>
        </h2>
        <p className="text-lg text-gray-600 mb-12">
          Have doubts? We’ve got you covered with quick answers.
        </p>
              ...
              className={`px-5 py-2 rounded-full border transition-all duration-300 font-medium text-sm md:text-base
                ${
                  selectedCategory === cat
                    ...
                    : "bg-white text-gray-800 border-gray-300 hover:border-[#D9070A]"
                }`}
            >
              {cat}
            </button>
        ...
        <div className="text-left space-y-4">
          {(faqData[selectedCategory] || []).map((faq, index) => (
            <div
              key={index}
              className="border border-gray-200 rounded-xl p-5 shadow-sm bg-gray-50 hover:shadow-md transition-all"
            >
              <button
                onClick={() =>
